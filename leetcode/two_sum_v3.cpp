...
class Solution{
    public:
        int find(vector<int> &nums, int start, int end, int target)
        {
            ...
            start++;// should ++, otherwise same value, [4, 0 4], 8
            while(start <= end)
            {
        ...
        vector<int> twoSum(vector<int>& nums, int target){
            ...
            for(int i = 0; i < size; i++)
            {
                if(o[i] == nums[start])
                {
                    start = i+1; o[i]++; break; //++ is a workaround to avoid same value
                }
            }
            ...
            for(int i = 0; i < size; i++)
            {
                if(o[i] == nums[found])
                {
                    found = i+1; o[i]++;break; //++ is a workaround to avoid same value
                }
            }
